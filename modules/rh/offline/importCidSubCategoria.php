...
$csvfile = $MIOLO->getConf('home.modules')."/rh/offline/cidsubcategoria.csv";
$db 	 = $MIOLO->getDatabase('ufjf');
$sql 	 = new sql('idcidcategoria,idcidsubcategoria,opc,descricao,restricaosexo','rh_cidsubcategoria');
$cs 	 = new csv2array();
$array 	 = $cs->getArray($csvfile);

foreach($array as $count=>$row)
{
   if (($count % 30) == 0) echo $row[0] . "\n";
   try
   {
       $db->execute($sql->insert($row));
   }
   catch (Exception $e)
   {
       echo $e->getMessage() . '\n';
