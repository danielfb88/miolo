...
function doTextTipo_result(result) 
{
    ...
    if (result) // Sugestão da EAFBarbacena
        xGetElementById('titulo').focus();
    xWalkTree(xGetElementById('m-container'), function(e){e.style.cursor = 'auto'});
}

function doTextInstituicao(url) 
{
	xWalkTree(xGetElementById('m-container'), function(e){e.style.cursor = 'wait'});
    origemExterna = xGetElementById("origemExterna");
    numProcesso = xGetElementById("numProcessoExterno");
    cp.set_response_type("TEXT");
    cp.call(url, "ajax_frmProcessoNovo_TextInstituicao", doTextInstituicao_result, origemExterna.checked);
}	
function doTextInstituicao_result(result) 
{
    xGetElementById('m_numProcesso').innerHTML = result;
	xWalkTree(xGetElementById('m-container'), function(e){e.style.cursor = 'auto'});
}
...
function doLimparDadosProcesso(url) 
{
